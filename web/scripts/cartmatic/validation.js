...
function createTip(uuid,errMsg){
	if(!$.isFunction($(document).PQuickTip)){//防止动态加载没有完成而出错
		setTimeout("createTip('"+uuid+"','"+errMsg+"')",500)
	}
	else{
		$("#validate_"+uuid).PQuickTip({
	    	source:'text',
	    	text:errMsg,
	    	title:'Error'
	    })
...
function __handleVaErrMsg(dom, errMsg) {
	if (errMsg && errMsg.length > 0){
		...
		var errorObj = $(dom);
		var lastChild = $("#validate_"+uuid);
	    if(lastChild.length==0){		
		    errorObj.addClass("x-form-invalid");
		    $("<img id='validate_"+uuid+"' src='"+__ctxPath+"/images/validate/exclamation.gif' border=0>").appendTo("body");
		    $("#validate_"+uuid).appendTo(errorObj.parent());
		    $("#validate_"+uuid).bind('remove',function(){
		    	$(this).PQuickTip.removeTip($(this));
		    })
	    }
	    if(!$.isFunction($(document).PQuickTip)){//防止多次加载QuickTip.js文件
	   ...
	   $("#validate_"+uuid).trigger("remove");
	   $("#validate_"+uuid).remove();
	}
	return errMsg;
}
