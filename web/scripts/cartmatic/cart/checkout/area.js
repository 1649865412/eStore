...
dsy.add("0_19_11",["莞城","石龙","虎门","万江","东城","南城","中堂","望牛墩","麻涌","石碣","高埗","道滘","沙田","厚街","长安","洪梅","寮步","大朗","大岭山","黄江","樟木头","清溪","塘厦","凤岗","常平","谢岗","桥头","横沥","东坑","企石","石排","茶山","松山湖","松山湖"]);
...
dsy.add("0_20_0",["青秀区","兴宁区","江南区s","西乡塘区","良庆区","邕宁区","武鸣县","横县","宾阳县","上林县","隆安县","马山县"]);
...
var s_u=["s_province_u","s_city_u","s_county_u"];//三个select的name
var opt0 = ["省份","地级市","市、县级市"];//初始值
var opt0_u = ["省份","地级市","市、县级市"];//初始值
function _init_area(){  //初始化函数
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
	  if(document.getElementById(s_u[i]))
		  document.getElementById(s_u[i]).onchange=new Function("change_u("+(i+1)+")");
	}
	change(0);
	change_u(0);
}
...
function change_u(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(s_u[i]).selectedIndex-1));
	};
	var ss=document.getElementById(s_u[v]);
	if(ss){
		with(ss){
			length = 0;
			options[0]=new Option(opt0_u[v],opt0_u[v]);
			if(v && document.getElementById(s_u[v-1]).selectedIndex>0 || !v){
				if(dsy.Exists(str)){
					ar = dsy.Items[str];
					for(i=0;i<ar.length;i++){
						options[length]=new Option(ar[i],ar[i]);
