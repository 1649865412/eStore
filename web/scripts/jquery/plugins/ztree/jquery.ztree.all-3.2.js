...
(function($j){
	...
	view = {
		addNodes: function(setting, parentNode, newNodes, isSilent) {
			...
			if (parentNode) {
				var target_switchObj = $j("#" + parentNode.tId + consts.id.SWITCH),
				target_icoObj = $j("#" + parentNode.tId + consts.id.ICON),
				target_ulObj = $j("#" + parentNode.tId + consts.id.UL);
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			if (node) {
				node.isAjaxing = true;
				var icoObj = $j("#" + node.tId + consts.id.ICON);
		...
		cancelPreSelectedNode: function (setting, node) {
			...
			for (var i=0, j=list.length-1; j>=i; j--) {
				if (!node || node === list[j]) {
					$j("#" + list[j].tId + consts.id.A).removeClass(consts.node.CURSELECTED);
		...
		createNodes: function(setting, level, nodes, parentNode) {
			...
			var root = data.getRoot(setting),
			childKey = setting.data.key.children,
			openFlag = !parentNode || parentNode.open || !!$j("#" + parentNode[childKey][0].tId).get(0);
			root.createdNodes = [];
			var zTreeHtml = view.appendNodes(setting, level, nodes, parentNode, true, openFlag);
			if (!parentNode) {
				setting.treeObj.append(zTreeHtml.join(''));
			} else {
				var ulObj = $j("#" + parentNode.tId + consts.id.UL);
				if (ulObj.get(0)) {
		...
		expandCollapseNode: function(setting, node, expandFlag, animateFlag, callback) {
			...
			if (!node.open && node.isParent && ((!$j("#" + node.tId + consts.id.UL).get(0)) || (node[childKey] && node[childKey].length>0 && !$j("#" + node[childKey][0].tId).get(0)))) {
				view.appendParentULDom(setting, node);
			}
			var ulObj = $j("#" + node.tId + consts.id.UL),
			switchObj = $j("#" + node.tId + consts.id.SWITCH),
			icoObj = $j("#" + node.tId + consts.id.ICON);
		...
		removeChildNodes: function(setting, node) {
			...
			if (!setting.data.keep.parent) {
				node.isParent = false;
				node.open = false;
				var tmp_switchObj = $j("#" + node.tId + consts.id.SWITCH),
				tmp_icoObj = $j("#" + node.tId + consts.id.ICON);
				view.replaceSwitchClass(node, tmp_switchObj, consts.folder.DOCU);
				view.replaceIcoClass(node, tmp_icoObj, consts.folder.DOCU);
				$j("#" + node.tId + consts.id.UL).remove();
			} else {
				$j("#" + node.tId + consts.id.UL).empty();
			}
		},
		removeNode: function(setting, node) {
			...
			if (!setting.data.keep.parent && parentNode[childKey].length < 1) {
				...
				parentNode.open = false;
				tmp_ulObj = $j("#" + parentNode.tId + consts.id.UL);
				tmp_switchObj = $j("#" + parentNode.tId + consts.id.SWITCH);
				tmp_icoObj = $j("#" + parentNode.tId + consts.id.ICON);
				...
				tmp_ulObj = $j("#" + newLast.tId + consts.id.UL);
				tmp_switchObj = $j("#" + newLast.tId + consts.id.SWITCH);
				tmp_icoObj = $j("#" + newLast.tId + consts.id.ICON);
				if (parentNode == root) {
					if (parentNode[childKey].length == 1) {
						//node was root, and ztree has only one root after move node
						view.replaceSwitchClass(newLast, tmp_switchObj, consts.line.ROOT);
					} else {
						var tmp_first_switchObj = $j("#" + parentNode[childKey][0].tId + consts.id.SWITCH);
		...
		selectNode: function(setting, node, addFlag) {
			...
			$j("#" + node.tId + consts.id.A).addClass(consts.node.CURSELECTED);
			data.addSelectedNode(setting, node);
		},
		setNodeFontCss: function(setting, treeNode) {
			var aObj = $j("#" + treeNode.tId + consts.id.A),
		...
		setNodeLineIcos: function(setting, node) {
			if (!node) return;
			var switchObj = $j("#" + node.tId + consts.id.SWITCH),
			ulObj = $j("#" + node.tId + consts.id.UL),
			icoObj = $j("#" + node.tId + consts.id.ICON),
		...
		setNodeName: function(setting, node) {
			...
			titleKey = data.getTitleKey(setting),
			nObj = $j("#" + node.tId + consts.id.SPAN);
			nObj.empty();
			...
			if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle) && node[titleKey]) {
				var aObj = $j("#" + node.tId + consts.id.A);
	...
	$j.fn.zTree = {
		...
		init: function(obj, zSetting, zNodes) {
			...
			$j.extend(true, setting, zSetting);
				...
				expandNode : function(node, expandFlag, sonSign, focus, callbackFlag) {
					...
					if (sonSign) {
						view.expandCollapseSonNode(this.setting, node, expandFlag, true, function() {
							if (focus !== false) {$j("#" + node.tId).focus().blur();}
						});
					} else {
...
(function($j){
	...
	_zTreeTools = function(setting, zTreeTools) {
		zTreeTools.checkNode = function(node, checked, checkTypeFlag, callbackFlag) {
			...
			if (tools.uCanDo(this.setting) && this.setting.check.enable && node.nocheck !== true) {
				...
				var checkObj = $j("#" + node.tId + consts.id.CHECK);
	...
	_handler = {
		...
		onMouseoverCheck: function(event, node) {
			if (node.chkDisabled === true) return false;
			var setting = data.getSetting(event.data.treeId),
			checkObj = $j("#" + node.tId + consts.id.CHECK);
				...
				if (node[checkedKey]) {
					if (setting.check.radioType == r.TYPE_ALL) {
						for (i = checkedList.length-1; i >= 0; i--) {
							...
							view.setChkClass(setting, $j("#" + pNode.tId + consts.id.CHECK), pNode);
	...
	$j.extend(true, $j.fn.zTree.consts, _consts);
	$j.extend(true, $j.fn.zTree._z, _z);
...
(function($j){
	...
	_zTreeTools = function(setting, zTreeTools) {
		...
		zTreeTools.moveNode = function(targetNode, node, moveType, isSilent) {
			...
			if (targetNode && !targetNode.isParent && setting.data.keep.leaf && moveType === consts.move.TYPE_INNER) {
				return null;
			} else if (targetNode && ((node.parentTId == targetNode.tId && moveType == consts.move.TYPE_INNER) || $j("#" + node.tId).find("#" + targetNode.tId).length > 0)) {
				return null;
			} else if (!targetNode) {
				targetNode = null;
			}
	...
	_handler = {
		...
		onMousedownNode: function(eventMouseDown, _node) {
			...
			var doc = $j(document), curNode, tmpArrow, tmpTarget,
			isOtherTree = false,
			...
			function _docMouseMove(event) {
				...
				if (root.dragFlag == 0) {
					...
					curNode = $j("<ul class='zTreeDragUL'></ul>");
					for (i=0, l=nodes.length; i<l; i++) {
						tmpNode = nodes[i];
						...
						tmpDom = $j("<li id='"+ tmpNode.tId +"_tmp'></li>");
						tmpDom.append($j("#" + tmpNode.tId + consts.id.A).clone());
						tmpDom.css("padding", "0");
						tmpDom.children("#" + tmpNode.tId + consts.id.A).removeClass(consts.node.CURSELECTED);
						curNode.append(tmpDom);
						if (i == setting.edit.drag.maxShowNodeNum-1) {
							tmpDom = $j("<li id='"+ tmpNode.tId +"_moretmp'><a>  ...  </a></li>");
							curNode.append(tmpDom);
							break;
						}
					}
					...
					tmpArrow = $j("<span class='tmpzTreeMove_arrow'></span>");
				...
				if (root.dragFlag == 1) {
					if (tmpTarget && tmpArrow.attr("id") == event.target.id && tmpTargetNodeId && (event.clientX + doc.scrollLeft()+2) > ($j("#" + tmpTargetNodeId + consts.id.A, tmpTarget).offset().left)) {
						var xT = $j("#" + tmpTargetNodeId + consts.id.A, tmpTarget);
						event.target = (xT.length > 0) ? xT.get(0) : event.target;
					} else if (tmpTarget) {
						tmpTarget.removeClass(consts.node.TMPTARGET_TREE);
						if (tmpTargetNodeId) $j("#" + tmpTargetNodeId + consts.id.A, tmpTarget).removeClass(consts.node.TMPTARGET_NODE + "_" + consts.move.TYPE_PREV)
					...
					for (var s in settings) {
						if (settings[s].treeId && settings[s].edit.enable && settings[s].treeId != setting.treeId
							&& (event.target.id == settings[s].treeId || $j(event.target).parents("#" + settings[s].treeId).length>0)) {
							isOtherTree = true;
					...
					if (event.target.id && targetSetting.treeObj.find("#" + event.target.id).length > 0) {
						...
						for (i=0, l=nodes.length; i<l; i++) {
							...
							if (targetObj.id === tmpNode.tId) {
								canMove = false;
								break;
							} else if ($j("#" + tmpNode.tId).find("#" + targetObj.id).length > 0) {
								canMove = false;
								break;
							}
						}
						if (canMove) {
							if (event.target.id &&
								(event.target.id == (targetObj.id + consts.id.A) || $j(event.target).parents("#" + targetObj.id + consts.id.A).length > 0)) {
								tmpTarget = $j(targetObj);
					...
					tmpNode = nodes[0];
					if (isTreeInner && (event.target.id == targetSetting.treeId || $j(event.target).parents("#" + targetSetting.treeId).length>0)) {
					...
					if (tmpTarget && tmpTarget.attr("id")!=targetSetting.treeId) {
						...
						if (!canPrev && !canNext && !canInner) {
							...
							var tmpTargetA = $j("#" + tmpTargetNodeId + consts.id.A, tmpTarget),
							tmpNextA = tmpTargetNode.isLastNode ? null : $j("#" + tmpTargetNode.getNextNode().tId + consts.id.A, tmpTarget.next()),
	...
	_tools = {
		...
		showIfameMask: function(setting, showSign) {
			...
			if (showSign) {
				//show mask
				var iframeList = $j("iframe");
				for (var i = 0, l = iframeList.length; i < l; i++) {
					var obj = iframeList.get(i),
					r = tools.getAbs(obj),
					dragMask = $j("<div id='zTreeMask_" + i + "' class='zTreeMask' style='background-color:yellow;opacity: 0.3;filter: alpha(opacity=30);    top:" + r[1] + "px; left:" + r[0] + "px; width:" + obj.offsetWidth + "px; height:" + obj.offsetHeight + "px;'></div>");
	...
	_view = {
		addEditBtn: function(setting, node) {
			if (node.editNameFlag || $j("#" + node.tId + consts.id.EDIT).length > 0) {
				return;
			}
			...
			$j("#" + node.tId + consts.id.EDIT).bind('click',
				function() {
		...
		addRemoveBtn: function(setting, node) {
			if (node.editNameFlag || $j("#" + node.tId + consts.id.REMOVE).length > 0) {
				return;
			}
			...
			$j("#" + node.tId + consts.id.REMOVE).bind('click',
				function() {
		...
		editNode: function(setting, node) {
			...
			view.selectNode(setting, node, false);
			$j("#" + node.tId + consts.id.SPAN).html("<input type=text class='rename' id='" + node.tId + consts.id.INPUT + "' treeNode" + consts.id.INPUT + " >");
			var inputObj = $j("#" + node.tId + consts.id.INPUT);
			...
			$j("#" + node.tId + consts.id.A).addClass(consts.node.CURSELECTED_EDIT);
		...
		moveNode: function(setting, targetNode, node, moveType, animateFlag, isSilent) {
			...
			if (targetNodeIsRoot) {
				...
				targetObj = $j("#" + targetNode.tId);
				target_ulObj = $j("#" + targetNode.tId + consts.id.UL);
			...
			if (!setting.data.keep.parent && oldParentNode[childKey].length < 1) {
				...
				var tmp_ulObj = $j("#" + oldParentNode.tId + consts.id.UL),
				tmp_switchObj = $j("#" + oldParentNode.tId + consts.id.SWITCH),
				tmp_icoObj = $j("#" + oldParentNode.tId + consts.id.ICON);
		...
		removeEditBtn: function(node) {
			$j("#" + node.tId + consts.id.EDIT).unbind().remove();
		},
		removeRemoveBtn: function(node) {
			$j("#" + node.tId + consts.id.REMOVE).unbind().remove();
		},
		...
		repairNodeLevelClass: function(setting, node, oldLevel) {
			...
			var liObj = $j("#" + node.tId),
			aObj = $j("#" + node.tId + consts.id.A),
			ulObj = $j("#" + node.tId + consts.id.UL),
