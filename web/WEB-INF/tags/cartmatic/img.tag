...
%><c:choose>
	...
	<c:otherwise>
		<img ${isLazyload?"original":"src"}="${mediaPath}<%=urlPath.toString()%>" <c:if test="${isLazyload}"> src="${resPath}/images/img/empty.gif"</c:if> onerror="$(this).attr('originsrc',this.src).attr('src',__defaultImage).removeAttr('onerror');" originsrc="" 
