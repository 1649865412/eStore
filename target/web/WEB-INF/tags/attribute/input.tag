<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %><%@ 
	...
	attribute name="cssName" type="java.lang.String" %><%@ 
...
<c:choose>
  ...
  <c:when test="${attribute.attributeDataType == 6}">
     <input type="checkbox" id="${aid}" <c:if test="${attr_value == 1 }">checked="checked"</c:if><c:if test="${not empty cssName }">class="${cssName }"</c:if> name="${attr_name}"/>&nbsp;
  ...
  <c:otherwise>
      <textarea name="${attr_name }" id="${aid}" <c:if test="${not empty cssName }">class="${cssName }"</c:if> rows="5" cols="15" <c:if test="${attribute.isRequired }">validConf="required"</c:if> ><c:out value="${attr_value==null?default_value:attr_value }"></c:out></textarea>
  </c:otherwise>    
</c:choose>
